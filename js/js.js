(function () {
    ...
    function eventShow(n) {
        for (var i = 0; i < evTotal; i++) {
            var el = document.getElementById('event-' + i);
            if (el) el.style.display = 'none';
        }
        var target = document.getElementById('event-' + n);
    ...
    function selectColor(el) {
        document.querySelectorAll('.color_dot').forEach(function(d) { d.classList.remove('selected'); });
        if (el) el.classList.add('selected');
    }

    function selectSize(el) {
        if (!el) return;
        var container = el.closest('.merch2_sizes');
        if (container) {
            container.querySelectorAll('.size_btn').forEach(function(b) { b.classList.remove('selected'); });
        }
    ...
    window.eventShow = eventShow;
    window.eventNext = eventNext;
    window.eventPrev = eventPrev;
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        (function() {
          ...
          function isMobile() {
    return window.innerWidth >= 360 && window.innerWidth <= 440;
  }
  
  function redirectToTarifs() {
    if (isMobile() && !window.location.pathname.includes('tarifs.html')) {
  ...
  redirectToTarifs();
  window.addEventListener('load', redirectToTarifs);
        ...
        (function() {
            ...
            const btn = document.getElementById('span_btn3');
            if (btn) {
